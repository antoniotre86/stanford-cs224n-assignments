...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        sents_ids = self.words2charindices(sents)
        sents_ids_padded = pad_sents_char(sents_ids, 0)
        sents_ids_padded_tensor = torch.tensor(sents_ids_padded, dtype=torch.long, device=device).permute(1, 0, 2)\
            .contiguous()
        return sents_ids_padded_tensor
        ### END YOUR CODE
