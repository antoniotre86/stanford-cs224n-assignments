...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        x = torch.index_select(self.embeddings, 0, w.flatten()).view(w.shape[0], self.n_features*self.embed_size)
    ...
    def forward(self, w):
        ...
        x = self.embedding_lookup(w)  # shape: batch_size x embed_size*n_features
        W = self.embed_to_hidden_weight  # shape: embed_size*n_features x hidden_size
        b1 = self.embed_to_hidden_bias  # shape: 1 x hidden_size
        eta = torch.matmul(x, W) + b1  # shape: batch_size x hidden_size
        h = F.relu(eta)
        h_d = self.dropout(h)
        U = self.hidden_to_logits_weight  # shape: hidden_size x n_classes
        b2 = self.hidden_to_logits_bias  # shape: 1 x n_classes
        logits = torch.matmul(h_d, U) + b2  # shape: batch_size x n_classes
