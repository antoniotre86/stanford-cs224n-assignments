...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition == 'S':
            ...
            head = self.stack[-1]
            dpn = self.stack[-2]
            self.dependencies.append((head, dpn))
            _ = self.stack.pop(-2)
        elif transition == 'RA':
            head = self.stack[-2]
            dpn = self.stack[-1]
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        ...
        for prs, transition in zip(minibatch_parses, transitions):
            prs.parse_step(transition)

            is_finished = (len(prs.buffer) == 0) and (len(prs.stack) == 1)
            if is_finished:
                _ = unfinished_parses.pop(unfinished_parses.index(prs))

    dependencies = [prs.dependencies for prs in partial_parses]
